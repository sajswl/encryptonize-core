...
OUT_DIR="${ROOT_DIR}/encryptonize/bootstrap/config"

mkdir -p "${OUT_DIR}"

# Get object cluster secrets
gcloud container clusters get-credentials $OBJ_CLUSTER --zone $ZONE --project $PROJECT

echo -e "${BLUE_ON}[+] Fetching object storage secrets${COLOR_OFF}"
...
# Bootstrap encryptonize
gcloud container clusters get-credentials $ENC_CLUSTER --zone $ZONE --project $PROJECT

echo -e "${BLUE_ON}[+] Bootstrapping encryptonize cluster${COLOR_OFF}"
kubectl apply -k "${ROOT_DIR}/encryptonize/bootstrap"
